            ...
            {blogs.map((blog) => (
              <div key={blog.id} className="flex flex-wrap items-center w-full xl:w-1/2 px-4 mb-8 md:-mx-4">
                ...
                <div className="w-full md:flex-1 md:px-4">
                  <Link href={`/blog/${blog.attributes.slug}`} className="inline-block mb-4 text-2xl leading-tight text-coolGray-800 hover:text-coolGray-900 font-bold hover:underline">
                    {blog.attributes.title}
                  </Link>
                  <p className="mb-4 text-sm text-coolGray-500">
                  ...
                  <Link href={`/blog/${blog.attributes.slug}`} className="inline-flex items-center text-base md:text-lg text-green-500 hover:text-green-600 font-semibold">
        ...
        {totalPages > 1 && (
          <div className="flex justify-center mx-auto max-w-max bg-white border border-coolGray-100 rounded-md shadow-lg">
            <button
              ...
              className="inline-flex items-center justify-center w-11 h-11 text-coolGray-300 hover:text-coolGray-400 font-medium border-r border-coolGray-100 disabled:opacity-50"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`inline-flex items-center justify-center w-11 h-11 font-medium border-r border-coolGray-100 ${currentPage === page ? 'text-green-500 bg-green-50' : 'text-coolGray-300 hover:text-green-500'}`}
              >
                {page}
              </button>
            ))}
            ...
            <button
              ...
              className="inline-flex items-center justify-center w-11 h-11 text-coolGray-300 hover:text-coolGray-400 font-medium disabled:opacity-50"
            >
